...
`broc` runs restore drills automatically. On a schedule, it restores a rotating sample of real files from real snapshots in your backup repository into scratch space, hashes them, and diffs that hash against a baseline recorded the first time it ever checked that exact file. Because snapshots are immutable, a baseline that stops matching means something is wrong - and broc fails loudly (non-zero exit code, structured JSON, a webhook, or a missed healthcheck ping) instead of leaving you to notice during an actual disaster.
...
`restic check` validates that the repository's pack files are internally consistent. It's necessary, but it never actually restores a file - it can't catch a broken restore path, a misconfigured target, permissions that silently prevent recovery, or a client that reads the repo fine but writes garbage back to disk. `broc` performs the full recovery round-trip a real disaster would require, on real data, and remembers whether it worked.

## Install

```sh
go install github.com/ravonhenson/broc-cli/cmd/broc@latest
```

Or build from source:

```sh
git clone https://github.com/ravonhenson/broc-cli
cd broc-cli
go build -o broc ./cmd/broc
```
...
```sh
# Point broc at a repo. Repository/password can also come from the
# usual RESTIC_REPOSITORY / RESTIC_PASSWORD_COMMAND env vars if you leave
# these flags off - broc works out of the box in any shell already set
# up to run restic.
broc init --name my-backups \
...
Put `broc run --config /path/to/my-backups.yaml` in cron/systemd-timer next to your backup job, pointed at a *different* schedule than the backup itself (there's no reason to restore-check a snapshot the moment it's created; the value is in continuously reverifying the whole history).

## How verification works

1. **Sample.** Each run picks up to `sample.files_per_run` (snapshot, file) pairs to check, preferring pairs that have never been verified, spread round-robin across snapshots so early runs get breadth rather than exhausting one snapshot first. Once everything has been seen at least once, broc starts reverifying the oldest-checked pairs (`sample.reverify_after_days`) to catch bit rot / silent storage corruption over the long run.
...
A mismatch is never silently "healed": once a baseline stops matching, broc keeps reporting the mismatch on every run until a human investigates and clears it.
...
| `2`  | Operational error: broc couldn't complete the run (backend unreachable, bad config, etc.) - it didn't get to check anything. |
...
`broc status` also exits non-zero if any file currently has an unresolved mismatch, so a monitoring check can catch a lingering problem even between runs.
...
notify:
  ...
  webhook:
    url: https://example.com/hooks/broc
...
scratch:
  dir: /var/tmp/broc/my-backups # default: $TMPDIR/broc/<name>
...
state:
  path: ~/.local/state/broc/my-backups.db
...
Run `broc init --help` to generate one of these interactively.

## Extensibility

`broc` talks to backup tools through a small `backend.Backend` interface (`internal/backend/backend.go`): list snapshots, list files in a snapshot, restore one file. The restic implementation is the reference (`internal/backend/restic`); adding Borg or Kopia is a matter of implementing that interface against their respective CLIs/APIs and registering it - no changes needed to sampling, state, verification, or reporting. Contributions for either are very welcome.
...
- **End-to-end tests** (`e2e/`) - build the actual `broc` binary and drive it as a subprocess exactly as a user would: `init`/`run`/`status`, exit codes 0/1/2, `--json` schema, webhook/healthcheck delivery, and two overlapping `run`s against the same state db failing cleanly instead of racing.
...
- [ ] `broc clear` to acknowledge/reset a mismatch after investigation
